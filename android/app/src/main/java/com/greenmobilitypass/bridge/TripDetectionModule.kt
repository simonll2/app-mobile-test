...
class TripDetectionModule(reactContext: ReactApplicationContext) :
    ...
    companion object {
        ...
        const val EVENT_TRANSITION_DETECTED = "onTransitionDetected"
    ...
    override fun getConstants(): MutableMap<String, Any> {
        return hashMapOf(
            ...
            "EVENT_TRANSITION_DETECTED" to EVENT_TRANSITION_DETECTED
        )
    }
    ...
    fun startDetection(promise: Promise) {
        try {
            ...
            // Set up trip detection listener with delay to ensure service is ready
            scope.launch {
                delay(500) // Wait 500ms for service to initialize
                setupTripListener()
                Log.d(TAG, "Trip listener setup completed")
            }
    ...
    private fun setupTripListener() {
        Log.d(TAG, "Setting up trip listener...")
        
        // Try to setup immediately if service is already running
        val service = TripDetectionService.getInstance()
        if (service != null) {
            Log.d(TAG, "Service instance found, setting up listeners immediately")
            service.onTripDetectedListener = { journey ->
                Log.d(TAG, "Trip detected, sending event to React Native")
                sendEvent(EVENT_TRIP_DETECTED, journeyToMap(journey))
            }
            
            // Setup transition listener for live tracking
            service.onTransitionDetectedListener = { activityType, transitionType ->
                Log.d(TAG, "Transition detected: $activityType -> $transitionType")
                val params = Arguments.createMap().apply {
                    putString("activityType", activityType)
                    putString("transitionType", transitionType)
                    putDouble("timestamp", System.currentTimeMillis().toDouble())
                }
                sendEvent(EVENT_TRANSITION_DETECTED, params)
            }
            
            // Setup GPS log listener for live tracking
            service.onGpsLogListener = { gpsLogData ->
                Log.d(TAG, "GPS log event: ${gpsLogData["event"]}")
                val params = Arguments.createMap().apply {
                    gpsLogData.forEach { (key, value) ->
                        when (value) {
                            is String -> putString(key, value)
                            is Number -> putDouble(key, (value as Number).toDouble())
                            is Boolean -> putBoolean(key, value)
                            else -> putString(key, value.toString())
                        }
                    }
                }
                sendEvent("onGpsLog", params)
            }
            Log.d(TAG, "Listeners configured successfully")
        } else {
            // Service not ready yet, will be set up when service starts
            // We'll set it up in a delayed way
            Log.d(TAG, "Service not ready, retrying...")
            scope.launch {
                var attempts = 0
                while (attempts < 15) { // Increased attempts
                    delay(300) // Wait 300ms between attempts
                    val serviceInstance = TripDetectionService.getInstance()
                    if (serviceInstance != null) {
                        Log.d(TAG, "Service instance found after ${attempts + 1} attempts, setting up listeners")
                        serviceInstance.onTripDetectedListener = { journey ->
                            Log.d(TAG, "Trip detected, sending event to React Native")
                            sendEvent(EVENT_TRIP_DETECTED, journeyToMap(journey))
                        }
                        
                        serviceInstance.onTransitionDetectedListener = { activityType, transitionType ->
                            Log.d(TAG, "Transition detected: $activityType -> $transitionType")
                            val params = Arguments.createMap().apply {
                                putString("activityType", activityType)
                                putString("transitionType", transitionType)
                        ...
                        // Setup GPS log listener for live tracking
                        serviceInstance.onGpsLogListener = { gpsLogData ->
                            Log.d(TAG, "GPS log event: ${gpsLogData["event"]}")
                            val params = Arguments.createMap().apply {
                                gpsLogData.forEach { (key, value) ->
                                    when (value) {
                        ...
                        Log.d(TAG, "Listeners configured successfully after retry")
                        return@launch
                    }
                    attempts++
                }
                Log.w(TAG, "Failed to setup listeners after ${attempts} attempts")
            }
        }
    }
